...
)
{
    ...
    *ResultantFormatLength = 0; // Default to zero
    ...
    ULONG resultChannels = g_CurrentGlobalChannels; 
    ...
    // Further checks for channels and bit depth against client's range (if needed)
    // For now, we primarily enforce the global sample rate and use global channels/bits.
    // Ensure the selected global format fits within the client's other constraints if they are strict.
    if (clientDataRangeAudio->MaximumChannels != 0 && resultChannels > clientDataRangeAudio->MaximumChannels) {
         DPF(DPF_LEVEL_INFO, ("DataRangeIntersection (Capture): Global channels %u > client max %u", 
            resultChannels, clientDataRangeAudio->MaximumChannels));
        return STATUS_NO_MATCH;
    }
     if (resultChannels < clientDataRangeAudio->MinimumChannels) {
        DPF(DPF_LEVEL_INFO, ("DataRangeIntersection (Capture): Global channels %u < client min %u", 
            resultChannels, clientDataRangeAudio->MinimumChannels));
        return STATUS_NO_MATCH;
    }
    // Similar for bit depth if it were configurable globally.
    if (resultBitsPerSample < clientDataRangeAudio->MinimumBitsPerSample || resultBitsPerSample > clientDataRangeAudio->MaximumBitsPerSample) {
        DPF(DPF_LEVEL_INFO, ("DataRangeIntersection (Capture): Global bit depth %u not in client range [%u, %u]",
            resultBitsPerSample, clientDataRangeAudio->MinimumBitsPerSample, clientDataRangeAudio->MaximumBitsPerSample));
        return STATUS_NO_MATCH;
    }


    // Ensure chosen format is within device's absolute min/max capabilities (from PcmAudioDataRange)
    if (resultChannels < MIN_CHANNELS_PCM || resultChannels > MAX_CHANNELS_PCM || // MIN_CHANNELS_PCM etc are from common.h
        resultBitsPerSample < MIN_BITS_PER_SAMPLE_PCM || resultBitsPerSample > MAX_BITS_PER_SAMPLE_PCM ||
        resultSampleRate < MIN_SAMPLE_RATE_PCM || resultSampleRate > MAX_SAMPLE_RATE_PCM) { // Device overall min/max
        DPF(DPF_LEVEL_INFO, ("DataRangeIntersection (Capture): Resulting format (%uCh, %ubit, %uHz) outside device capabilities.",
    ...
    pResFormatWfx->DataFormat.SampleSize = 0; 
