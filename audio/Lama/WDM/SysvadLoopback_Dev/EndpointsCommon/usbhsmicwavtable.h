...
// Add the missing function prototype
NTSTATUS PropertyHandler_WaveFilter(_In_ PPCPROPERTY_REQUEST PropertyRequest);

// Define the macro if not already defined
#ifndef DEFINE_PCAUTOMATION_TABLE_PROP
#define DEFINE_PCAUTOMATION_TABLE_PROP(name, prop) \
const PCAUTOMATION_TABLE_PROP name = { prop, SIZEOF_ARRAY(prop) }
#endif
...
PIN_DEVICE_FORMATS_AND_MODES UsbHsMicPinDeviceFormatsAndModes[] = 
{
    ...
    },
    {
        ...
        UsbHsMicPinSupportedDeviceFormats, // WaveFormats
        SIZEOF_ARRAY(UsbHsMicPinSupportedDeviceFormats), // WaveFormatsCount
        UsbHsMicPinSupportedDeviceModes, // ModeAndDefaultFormat
        SIZEOF_ARRAY(UsbHsMicPinSupportedDeviceModes) // ModeAndDefaultFormatCount
    }
};
