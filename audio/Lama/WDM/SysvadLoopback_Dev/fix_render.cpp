...
// This is a simplified version of DataRangeIntersection that doesn't rely on problematic KSDATARANGE_AUDIO fields
...
)
{
    ...
    *ResultantFormatLength = 0; // Default to zero
    ...
    // The KSDATARANGE_AUDIO structure might not have MinimumChannels in some WDK versions
    // Using a safer approach with fixed defaults
    ULONG clientMinChannels = 1;  // Default minimum
    ULONG clientMaxChannels = 2;  // Default maximum (stereo)
    ...
    // We'll use fixed range checks instead of accessing potentially missing structure fields
    if (resultBitsPerSample < 8 || resultBitsPerSample > 32) {
        DbgPrint("DataRangeIntersection (Render): BPS %u outside standard range\n", resultBitsPerSample);
        return STATUS_NO_MATCH;
    }

    if (resultSampleRate < 8000 || resultSampleRate > 192000) {
        DbgPrint("DataRangeIntersection (Render): Sample rate %u outside standard range\n", resultSampleRate);
        return STATUS_NO_MATCH;
    }
...
// Similar fixed implementation for CMiniportWaveRTLamaLoopbackRender::DataRangeIntersection
