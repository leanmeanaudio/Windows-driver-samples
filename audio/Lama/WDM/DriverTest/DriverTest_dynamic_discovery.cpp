...
bool GetDevicePath(const GUID& interfaceGuid, const std::wstring& nameSubstring, std::wstring& outDevicePath) {
    ...
    for (DWORD i = 0; SetupDiEnumDeviceInterfaces(hDevInfo, NULL, &interfaceGuid, i, &deviceInterfaceData); ++i) {
        ...
        if (GetLastError() != ERROR_INSUFFICIENT_BUFFER) {
            ...
            // std::cerr << "Failed to get required size for device interface detail. Error: " << GetLastError() << std::endl;
            // continue; // Actually, this error means something else went wrong if it's not ERROR_INSUFFICIENT_BUFFER
        }
        ...
        // Check friendly name for the substring first
        ...
        // Fallback to checking device path if friendly name doesn't match or is empty
        // This is less ideal as device paths can be cryptic.
        // A more robust approach for device path check might involve parsing it for known patterns if needed.
        // For now, a simple substring search on the path.
        // Convert nameSubstring to lowercase for case-insensitive search in path
...
void testAudioLoopback(LPCWSTR renderDevicePath, LPCWSTR captureDevicePath, uint16_t numChannelsToTest, const std::string& wavFileSuffix) {
    ...
    formatExt.WaveFormatEx.nSamplesPerSec = 48000; // Assuming 48kHz for now, SR control test is separate
    ...
    formatExt.dwChannelMask = (numChannelsToTest == 2) ? KSAUDIO_SPEAKER_STEREO : 0; // Basic mask, for 16ch will be 0
    ...
    if (chunkSizeBytes == 0 && dataByteSize > 0) chunkSizeBytes = dataByteSize; // Avoid infinite loop if chunk is 0
    ...
    while ((totalBytesRead < dataByteSize || totalBytesWritten < dataByteSize) && chunkSizeBytes > 0) {
        ...
        if (bytesToReadThisChunk > 0 && totalBytesWritten > totalBytesRead) { 
             // Add a small delay to allow data to loop through the driver
            Sleep(bufferDurationMs / 2); // Wait for half the buffer duration
        ...
        if (bytesToWriteThisChunk == 0 && bytesToReadThisChunk == 0) break; // Safety break
    }
...
int main() {
    ...
    // Attempt to find Render device (try common names for LAMA)
    if (GetDevicePath(KSCATEGORY_AUDIO, L"Lama Loopback Render", renderDevicePath)) {
        renderDeviceFound = true;
    } else if (GetDevicePath(KSCATEGORY_AUDIO, L"SysvadLoopback_Dev", renderDevicePath)) { // Fallback name
        renderDeviceFound = true;
    } else if (GetDevicePath(KSCATEGORY_AUDIO, L"LAMA", renderDevicePath)) { // Generic LAMA
         renderDeviceFound = true;
    }
    ...
    // Attempt to find Capture device
    if (GetDevicePath(KSCATEGORY_AUDIO, L"Lama Loopback Capture", captureDevicePath)) {
        captureDeviceFound = true;
    } else if (GetDevicePath(KSCATEGORY_AUDIO, L"SysvadLoopback_Dev", captureDevicePath)) { // Fallback name
         captureDeviceFound = true;
    } else if (GetDevicePath(KSCATEGORY_AUDIO, L"LAMA", captureDevicePath)) { // Generic LAMA
         captureDeviceFound = true;
    }
    ...
    if (renderDeviceFound && captureDeviceFound) {
        ...
        std::wcout << L"------------------------------------" << std::endl;
        // testAudioLoopback16ch will be added in a subsequent step
    } else {
        std::wcout << L"Skipping Audio Loopback test as Render or Capture device was not found." << std::endl;
        std::wcout << L"------------------------------------" << std::endl;
    }
