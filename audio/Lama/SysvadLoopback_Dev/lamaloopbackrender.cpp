...
)
{
    ...
    *ResultantFormatLength = 0; // Default to zero
    ...
    {
        DPF(DPF_LEVEL_INFO, ("DataRangeIntersection (Render): No match due to Major/Sub/Specifier GUIDs."));
    ...
    // If clientMaxChannels is 0, it often means "any" or "up to device capability".
    // Cap at driver's absolute max (MAX_CHANNELS_PCM from common.h).
    if (clientMaxChannels == 0 || clientMaxChannels > MAX_CHANNELS_PCM) {
        clientMaxChannels = MAX_CHANNELS_PCM;
    }
   
    // Ensure minChannels is not greater than maxChannels requested by client.
    if (clientMinChannels > clientMaxChannels) {
        DPF(DPF_LEVEL_INFO, ("DataRangeIntersection (Render): Client min channels %u > client max channels %u. No match.", clientMinChannels, clientMaxChannels));
        return STATUS_NO_MATCH; 
    }
    
    // Try to use the client's maximum preferred channels if it's within our device capabilities.
    // If not, try their minimum.
    if (clientMaxChannels >= MIN_CHANNELS_PCM && clientMaxChannels <= MAX_CHANNELS_PCM) {
        ...
        resultChannels = clientMinChannels;
         ...
         DPF(DPF_LEVEL_INFO, ("DataRangeIntersection (Render): Client channel range [%u, %u] is outside device capabilities [%u, %u]. No match.",
             clientMinChannels, clientMaxChannels, MIN_CHANNELS_PCM, MAX_CHANNELS_PCM));
        return STATUS_NO_MATCH;
    }
   
    // Final validation against absolute device capabilities (this step is crucial)
    if (resultChannels < MIN_CHANNELS_PCM || resultChannels > MAX_CHANNELS_PCM) {
        DPF(DPF_LEVEL_INFO, ("DataRangeIntersection (Render): Selected resultChannels %u is outside device capabilities [%u, %u]. No match.",
    ...
    // Validate BitsPerSample against client's range
    ...
    if (resultBitsPerSample < MIN_BITS_PER_SAMPLE_PCM || resultBitsPerSample > MAX_BITS_PER_SAMPLE_PCM ||
        ...
        DPF(DPF_LEVEL_INFO, ("DataRangeIntersection (Render): Resulting format SR/BPS (%uHz, %ubit) outside device capabilities.",
    ...
    if (pWfx->Format.nChannels == 1) {
        ...
        // For >2 channels, a specific mask should be used if known, otherwise 0.
        // For testing, 0 might be acceptable if the driver/OS can infer a default.
        pWfx->dwChannelMask = 0; 
    }
    pWfx->SubFormat = KSDATAFORMAT_SUBTYPE_PCM;

    *ResultantFormatLength = sizeof(KSDATAFORMAT_WAVEFORMATEXTENSIBLE);
    DPF(DPF_LEVEL_INFO, ("DataRangeIntersection (Render): Success. Format: %uHz, %uch, %ubit", 
