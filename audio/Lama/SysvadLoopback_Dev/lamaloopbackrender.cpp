...
NTSTATUS CMiniportTopologyLamaLoopbackRender::DataRangeIntersection(_In_ ULONG PinId, _In_ PKSDATARANGE DataRange, _In_ PKSDATARANGE MatchingDataRange, _In_ ULONG OutputBufferLength, _Out_writes_bytes_to_opt_(OutputBufferLength, *ResultantFormatLength) PVOID ResultantFormat, _Out_ PULONG ResultantFormatLength) { /* ... existing full implementation ... */ 
    PAGED_CODE(); DPF_ENTER(("[CMiniportTopologyLamaLoopbackRender::DataRangeIntersection] PinId=%u", PinId)); UNREFERENCED_PARAMETER(MatchingDataRange); if (!DataRange || !ResultantFormatLength) { return STATUS_INVALID_PARAMETER; } *ResultantFormatLength = 0; 
    if (!IsEqualGUIDAligned(DataRange->MajorFormat, KSDATAFORMAT_TYPE_AUDIO) || !IsEqualGUIDAligned(DataRange->SubFormat, KSDATAFORMAT_SUBTYPE_PCM) || !IsEqualGUIDAligned(DataRange->Specifier, KSDATAFORMAT_SPECIFIER_WAVEFORMATEX)) { DPF(DPF_LEVEL_INFO, ("DataRangeIntersection (Render): No match due to Major/Sub/Specifier GUIDs.")); return STATUS_NO_MATCH; }
    ...
    if (clientMaxChannels == 0 || clientMaxChannels > MAX_CHANNELS_PCM) { clientMaxChannels = MAX_CHANNELS_PCM; } if (clientMinChannels > clientMaxChannels) { DPF(DPF_LEVEL_INFO, ("DataRangeIntersection (Render): Client min channels %u > client max channels %u. No match.", clientMinChannels, clientMaxChannels)); return STATUS_NO_MATCH; }
    if (clientMaxChannels >= MIN_CHANNELS_PCM && clientMaxChannels <= MAX_CHANNELS_PCM) { resultChannels = clientMaxChannels; } else if (clientMinChannels >= MIN_CHANNELS_PCM && clientMinChannels <= MAX_CHANNELS_PCM) { resultChannels = clientMinChannels; } else { DPF(DPF_LEVEL_INFO, ("DataRangeIntersection (Render): Client channel range [%u, %u] is outside device capabilities [%u, %u]. No match.", clientMinChannels, clientMaxChannels, MIN_CHANNELS_PCM, MAX_CHANNELS_PCM)); return STATUS_NO_MATCH; }
    if (resultChannels < MIN_CHANNELS_PCM || resultChannels > MAX_CHANNELS_PCM) { DPF(DPF_LEVEL_INFO, ("DataRangeIntersection (Render): Selected resultChannels %u is outside device capabilities [%u, %u]. No match.", resultChannels, MIN_CHANNELS_PCM, MAX_CHANNELS_PCM)); return STATUS_NO_MATCH; }
    ULONG resultBitsPerSample = g_CurrentGlobalBitsPerSample; ULONG resultSampleRate = g_CurrentGlobalSampleRate;      
    if (resultBitsPerSample < clientDataRangeAudio->MinimumBitsPerSample || resultBitsPerSample > clientDataRangeAudio->MaximumBitsPerSample) { DPF(DPF_LEVEL_INFO, ("DataRangeIntersection (Render): Global BPS %u not in client range [%u, %u]", resultBitsPerSample, clientDataRangeAudio->MinimumBitsPerSample, clientDataRangeAudio->MaximumBitsPerSample)); return STATUS_NO_MATCH; }
    if (resultSampleRate < clientDataRangeAudio->MinimumSampleRate || resultSampleRate > clientDataRangeAudio->MaximumSampleRate) { DPF(DPF_LEVEL_INFO, ("DataRangeIntersection (Render): Global SR %u not in client range [%u, %u]", resultSampleRate, clientDataRangeAudio->MinimumSampleRate, clientDataRangeAudio->MaximumSampleRate)); return STATUS_NO_MATCH; }
    if (resultBitsPerSample < MIN_BITS_PER_SAMPLE_PCM || resultBitsPerSample > MAX_BITS_PER_SAMPLE_PCM || resultSampleRate < MIN_SAMPLE_RATE_PCM || resultSampleRate > MAX_SAMPLE_RATE_PCM) { DPF(DPF_LEVEL_INFO, ("DataRangeIntersection (Render): Resulting format SR/BPS (%uHz, %ubit) outside device capabilities.", resultSampleRate, resultBitsPerSample)); return STATUS_NO_MATCH; }
    if (!ResultantFormat) { *ResultantFormatLength = sizeof(KSDATAFORMAT_WAVEFORMATEXTENSIBLE); return STATUS_BUFFER_OVERFLOW; } if (OutputBufferLength < sizeof(KSDATAFORMAT_WAVEFORMATEXTENSIBLE)) { DPF(DPF_LEVEL_ERROR, ("DataRangeIntersection: Output buffer too small. Needed %u, Got %u", (ULONG)sizeof(KSDATAFORMAT_WAVEFORMATEXTENSIBLE), OutputBufferLength)); return STATUS_BUFFER_TOO_SMALL; }
    ...
    *ResultantFormatLength = sizeof(KSDATAFORMAT_WAVEFORMATEXTENSIBLE); DPF(DPF_LEVEL_INFO, ("DataRangeIntersection (Render): Success. Format: %uHz, %uch, %ubit", pWfx->Format.nSamplesPerSec, pWfx->Format.nChannels, pWfx->Format.wBitsPerSample)); return STATUS_SUCCESS;
}
